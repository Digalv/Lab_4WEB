...
if (!!getLS('data_coords')) {
    boxes = getLS('data_coords');
    boxGenerator(boxes);
}
...
function boxGenerator(list) {
    ...
    for (let i = 0; i < list.length; i++) {
        template += '<div class="box" style="left: ' + list[i].x + 'px; top: ' + list[i].y + 'px;" data-index="' + i + '"><p class="note">Notes</p><textarea class="text" text-index="' + i + '" rows="10" cols="30" placeholder="Введите текст :" >' + list[i].text + '</textarea></div>';
    }
